...
soc_vec1 <- sub_pca |> 
  ...
  #filter(freq_ford == "Chemical sciences") |> 
...
sd_pos <- stats_pos$sd_pos*2
mean_neg <- stats_neg$mean_neg
sd_neg <- stats_neg$sd_neg*2
...
soc_cos |> 
  ...
  mutate(topic_desc = stringr::word(topic_desc)) |> 
  mutate(abs_cosine = abs(cosine)) |> 
  mutate(pole = ifelse(cosine > 0, "pos", "neg")) |> 
  ggplot(aes(PC1, PC2)) +
  geom_text(aes(label = topic_desc, size = abs_cosine, color = pole), check_overlap = TRUE)
