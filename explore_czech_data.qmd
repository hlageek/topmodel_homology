...
econ_pca <- sub_pca |> 
  filter(freq_ford == "Physical sciences") |> 
  pull(pca) |> 
  pluck(1)
pc1_scores <- econ_pca$x[, 1]
econ_vedidks <- pca_projection |> 
select(freq_ford, vedidk) |> 
  filter(freq_ford == "Physical sciences") |> 
  pull(vedidk)
names(pc1_scores) <- econ_vedidks
contributions_pc1 <- pc1_scores^2
avg_contribution_pc1 <- mean(contributions_pc1)
indices_above_avg <- which(contributions_pc1 > avg_contribution_pc1)
positive_indices <- indices_above_avg[pc1_scores[indices_above_avg] > 0]
positive_contributors <- tibble(
  vedidk = names(pc1_scores)[positive_indices],
  PC1_score = pc1_scores[positive_indices],
  contribution = contributions_pc1[positive_indices],
  direction = "positive"
) |> 
  arrange(desc(contribution))

negative_indices <- indices_above_avg[pc1_scores[indices_above_avg] < 0]
negative_contributors <- data.frame(
  vedidk = names(pc1_scores)[negative_indices],
  PC1_score = pc1_scores[negative_indices],
  contribution = contributions_pc1[negative_indices],
  direction = "negative"
) |> 
  arrange(desc(contribution))

contributors <- bind_rows(positive_contributors, negative_contributors)
```
