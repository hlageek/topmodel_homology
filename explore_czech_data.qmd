...
```{r}
people_pubs_portfolios <-  arrow::read_feather(people_pubs_data$path) |>
      select(vedidk, id_unique) |>
      inner_join(
        arrow::read_feather(docmodel_data$path) |> 
...
people_portfolios <- people_pubs_portfolios |>
  tidytable::summarise(tidytable::across(tidytable::starts_with("topic"), mean), .by = "vedidk") |> 
  as_tibble()
